import type { GrammarLessonSegment, GrammarUnit } from "@/features/grammar/types";

/**
 * Deterministic audio lesson script from validated grammar unit content.
 * Transcript and TTS must share this exact object.
 */
export function buildGrammarLessonScript(
  unit: GrammarUnit,
): GrammarLessonSegment[] {
  const segments: GrammarLessonSegment[] = [];
  let order = 0;

  const push = (
    type: GrammarLessonSegment["type"],
    text: string,
    pauseAfterMs = 400,
  ) => {
    segments.push({
  ...
  push(
    "task_lead_in",
    "When you are ready, try the short practice task on this page.",
    500,
  );
