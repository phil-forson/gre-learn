import type { SentenceLessonSegment, SentenceUnit } from "@/features/sentence/types";

/**
 * Deterministic audio lesson script from validated sentence unit content.
 * Transcript and TTS must share this exact object.
 */
export function buildSentenceLessonScript(
  unit: SentenceUnit,
): SentenceLessonSegment[] {
  const segments: SentenceLessonSegment[] = [];
  let order = 0;

  const push = (
    type: SentenceLessonSegment["type"],
    text: string,
    pauseAfterMs = 400,
  ) => {
    segments.push({
  ...
  push(
    "task_lead_in",
    "When you are ready, try the short writing practice on this page.",
    500,
  );
