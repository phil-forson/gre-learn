...
export function SentenceAudioPlayer({ unitId, onPlayed }: Props) {
  ...
  const signalPlay = useCallback(() => {
    ...
    void fetch(`/api/sentence/units/${unitId}/play`, { method: "POST" })
      .then((res) => {
        if (res.ok) onPlayed?.();
      })
      .catch(() => undefined);
  ...
  const loadLesson = useCallback(async () => {
    ...
    setPlaying(false);
    try {
      const response = await fetch("/api/sentence/audio/generate", {
        method: "POST",
