...
export function SpeakingAudioPlayer({ unitId, onPlayed }: Props) {
  ...
  const signalPlay = useCallback(() => {
    ...
    void fetch(`/api/speaking/units/${unitId}/play`, { method: "POST" })
      .then((res) => {
        if (res.ok) onPlayed?.();
      })
      .catch(() => undefined);
  ...
  const loadLesson = useCallback(async () => {
    ...
    setPlaying(false);
    try {
      const response = await fetch("/api/speaking/audio/generate", {
        method: "POST",
