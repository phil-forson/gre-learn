...
export default function SentenceUnitPage() {
  const params = useParams<{ unitId: string }>();
  const unitId = params.unitId;
  const [unit, setUnit] = useState<PublicSentenceUnit | null>(null);
  const [progress, setProgress] = useState<SentenceProgress | null>(null);
  ...
  const refresh = useCallback(async () => {
    const res = await fetch(`/api/sentence/units/${unitId}`);
    ...
    startTransition(async () => {
      try {
        ...
        const res = await fetch(
          `/api/sentence/units/${unit.id}/micro-task`,
          {
            method: "POST",
        ...
        setScore(nextScore);
        setProgress(data.progress as SentenceProgress);
  ...
  if (!unit) {
    return (
      <div className="space-y-4">
        ...
        <Link href="/path/tracks/sentence" className="text-sm text-[var(--accent)]">
          ← Sentence track
        </Link>
      </div>
    );
    ...
    <div className="mx-auto max-w-2xl space-y-10">
      <div>
        <p className="font-[family-name:var(--font-ui)] text-xs uppercase tracking-[0.16em] text-[var(--ink-muted)]">
          Sentence · {unit.cefrBand} · en-US
        </p>
        <div className="mt-1 flex flex-wrap items-center gap-3">
          <h1 className="font-[family-name:var(--font-display)] text-3xl font-semibold tracking-tight">
            {unit.title}
          </h1>
          <SentenceCompletedBadge completed={Boolean(completed)} size="md" />
        </div>
