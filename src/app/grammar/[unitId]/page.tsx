...
import { GrammarAudioPlayer } from "@/features/grammar/components/GrammarAudioPlayer";
    ...
    startTransition(async () => {
      try {
        ...
        const res = await fetch(
          `/api/grammar/units/${unit.id}/micro-task`,
          {
            method: "POST",
  ...
  if (!unit) {
    return (
      <div className="space-y-4">
        ...
        <Link href="/path/tracks/grammar" className="text-sm text-[var(--accent)]">
          ← Grammar track
        </Link>
      </div>
    );
    ...
    <div className="mx-auto max-w-2xl space-y-10">
      <div>
        <p className="font-[family-name:var(--font-ui)] text-xs uppercase tracking-[0.16em] text-[var(--ink-muted)]">
          Grammar · {unit.cefrBand} · en-US
        </p>
        <div className="mt-1 flex flex-wrap items-center gap-3">
          <h1 className="font-[family-name:var(--font-display)] text-3xl font-semibold tracking-tight">
            {unit.title}
          </h1>
          <GrammarCompletedBadge completed={Boolean(completed)} size="md" />
        </div>
